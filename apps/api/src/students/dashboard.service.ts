...
export class DashboardService {
  ...
  async getOverview(userId: string) {
    const student = await this.prisma.student.findUnique({
      ...
      include: {
        ...
        examRecords: {
          select: { id: true, examDate: true, result: true },
          take: 1,
          orderBy: { createdAt: 'desc' },
        },
      },
    ...
    const [documentStats, paymentStats] = await Promise.all([
      this.prisma.studentDocument.groupBy({
        by: ['status'],
    ...
    for (const p of paymentStats) {
      ...
      if (p.status === 'SUCCESS' || p.status === 'MANUALLY_APPROVED')
        payStats.totalPaid += p._sum.amount || 0;
      else if (p.status === 'PENDING')
        payStats.pendingAmount += p._sum.amount || 0;
    }
    ...
    return {
      ...
      lettersAvailability: {
        ...
        invitationLetter: !!student.invitationLetter,
      },
    };
  }
  ...
  async getActivity(userId: string) {
    ...
    const [recentEvents, unreadNotifications, examRecords, visaApps] =
      await Promise.all([
    ...
    const upcomingDeadlines = [
      ...examRecords.map((e) => ({
        type: 'exam' as const,
        date: e.examDate,
        title: `Exam: ${e.examSubject || 'Entrance'}`,
        detail: e.examCenter,
      })),
      ...visaApps.map((v) => ({
        type: 'visa' as const,
  ...
  async getNextSteps(userId: string) {
    const student = await this.prisma.student.findUnique({
      ...
      include: {
        ...
        documents: {
          select: {
            status: true,
            documentType: { select: { requiredForStage: true } },
          },
        },
    ...
    actions.push({
      ...
      completed: passportComplete,
    });

    if (stage >= 1) {
      const stageDocs = student.documents.filter(
        (d) => d.documentType.requiredForStage <= stage,
      );
      const approvedDocs = stageDocs.filter(
        (d) => d.status === 'APPROVED',
      ).length;
      actions.push({
        type: 'documents',
        title: 'Upload required documents',
        description: `${approvedDocs}/${stageDocs.length || 1} documents approved`,
        actionUrl: '/student/documents',
        priority: 'high',
        completed: stageDocs.length > 0 && approvedDocs === stageDocs.length,
      });

      const stagePayment = student.payments.find(
        (p) =>
          p.stage === stage &&
      ...
      actions.push({
        type: 'payment',
        title: `Complete Stage ${stage} payment`,
        description: stagePayment
          ? 'Payment confirmed'
          : 'Payment required to proceed',
        actionUrl: '/student/payments',
        priority: 'high',
        completed: !!stagePayment,
      });
    }
    ...
    if (stage >= 1) {
      ...
      actions.push({
        ...
        description: hasApp
          ...
          : 'Apply to your preferred university',
    ...
    if (stage >= 2) {
      ...
      actions.push({
        ...
        description: passed ? 'Exam passed!' : 'Prepare for entrance exam',
    ...
    if (stage >= 3) {
      actions.push({
        ...
        description: student.admissionLetter
          ? 'Available for download'
          : 'Awaiting admin upload',
        actionUrl: '/student/letters',
        priority: 'medium',
        completed: !!student.admissionLetter,
      });
    }
    ...
    if (stage >= 4) {
      actions.push({
        ...
        description: student.invitationLetter?.isDownloadable
          ? 'Ready to download'
          : 'Pending approval',
        actionUrl: '/student/letters',
        priority: 'high',
        completed: !!student.invitationLetter?.isDownloadable,
      });
    }
    ...
    if (stage >= 5) {
      actions.push({
        ...
        description: 'Start your visa process',
        actionUrl: '/student/visa-support',
        priority: 'high',
        completed: false,
      });
    }
    ...
    const completionPercentage =
      actions.length > 0 ? Math.round((completed / actions.length) * 100) : 0;
