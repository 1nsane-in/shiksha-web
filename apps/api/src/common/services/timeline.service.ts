...
export class TimelineService {
  ...
  async getApplicationTimeline(applicationId: string) {
    ...
    return events.map((event) => ({
      ...
      isCompleted: true,
      isActive:
        event.stage === latestStage &&
        event.occurredAt.getTime() === latestTime,
    }));
  }
  ...
  async onApplicationSubmitted(applicationId: string, studentId: string) {
    return this.createEvent({
      ...
      description:
        'Your university application has been submitted successfully.',
      metadata: { submittedAt: new Date().toISOString() },
    });
  }

  async onStageAdvanced(
    applicationId: string,
    studentId: string,
    fromStage: number,
    toStage: number,
    ...
    return this.createEvent({
      ...
      title:
        ...
        toStage +
        ' Unlocked: ' +
        (names[toStage] || 'Stage ' + toStage),
      description:
        'You have progressed from "' +
        (names[fromStage] || 'Stage ' + fromStage) +
        '" to "' +
        (names[toStage] || 'Stage ' + toStage) +
        '".',
  ...
  async onAdmissionLetterUploaded(applicationId: string, studentId: string) {
    return this.createEvent({
      ...
      description:
        'Your admission letter has been uploaded. Please review and proceed with payment.',
      metadata: { uploadedAt: new Date().toISOString() },
    });
  }

  async onStage2PaymentCompleted(
    applicationId: string,
    studentId: string,
    amount: number,
  ) {
    return this.createEvent({
      ...
      metadata: { amount, paidAt: new Date().toISOString() },
    });
  }

  async onExamScheduled(
    applicationId: string,
    studentId: string,
    examDate: string,
    examCenter: string,
    ...
    return this.createEvent({
      ...
      description:
        'Your exam is scheduled on ' + examDate + ' at ' + examCenter + '.',
      metadata: { examDate, examCenter },
    });
  }

  async onExamResultDeclared(
    applicationId: string,
    studentId: string,
    passed: boolean,
    remarks?: string,
    ...
    return this.createEvent({
      ...
      description: passed
        ...
        : remarks ||
          'You did not pass the entrance exam. Please contact support for next steps.',
      metadata: { passed, remarks, declaredAt: new Date().toISOString() },
    });
  }
  ...
  async onInvitationLetterUploaded(applicationId: string, studentId: string) {
    return this.createEvent({
      ...
      studentId,
      stage: 4,
      event: 'INVITATION_LETTER_ISSUED',
      title: 'Invitation Letter Issued',
      description: 'Your invitation letter is now available for download.',
      metadata: { uploadedAt: new Date().toISOString() },
    });
  }
  ...
  async onVisaSupportStarted(applicationId: string, studentId: string) {
    return this.createEvent({
      ...
      studentId,
      stage: 5,
      event: 'VISA_SUPPORT_STARTED',
      title: 'Visa Support Started',
      description:
        'Visa application support is now available. Please check the visa checklist.',
  ...
  async onTicketResolved(
    ...
    return this.createEvent({
      ...
      description: 'A support ticket has been resolved.',
      metadata: { ticketId },
    });
  }
  ...
  async onApplicationCompleted(applicationId: string, studentId: string) {
    return this.createEvent({
      ...
      description: 'All stages completed. Welcome aboard!',
